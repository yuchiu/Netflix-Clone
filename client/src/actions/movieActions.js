...
export default {
  ...
  fetchTheMovie: movieId => dispatch => {
    axios
  ...
  }, // return // fetchTheMovie
  fetchCast: movieId => dispatch => {
    axios
  ...
  fetchSearch: searchText => {
    if (searchText.length >= 1) {
      return dispatch => {
        axios
          .get(
            `${ROOT_URL}/search/movie?${API_KEY}&language=en-US&query=${searchText}&page=1&include_adult=false`
          )
          .then(response => {
